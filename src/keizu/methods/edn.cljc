(ns keizu.methods.edn
  "Minimal EDN reader (subset: [] {} :kw \"str\" num bool nil). ADR-2606066000.
  ...
  STRINGS, NOT as Clojure keywords. weave.py / social.py / autorun.py / the charter-
  invariant tests key every seed record on string keys (\":node/scope\", \":rel/kind\",
  \":money/kind\", …) and the Python `:`-strings stay strings (root CLAUDE.md
  ...
  Matches the ake reference (`ake.methods._edn`) — keizu's `_edn.py` is the same
  family (logic byte-identical; only the docstring differs).
...
(defn- next-tok! [state]
  (let [ts @state]
    (when (empty? ts)
      (throw (ex-info "keizu.methods.edn: unexpected end of input" {})))
    (reset! state (rest ts))
    (first ts)))
