...
3. **No commercial gov-intelligence terminals (Charter Rider §2(e), N5).** The following are
   ...
   Crunchbase, LexisNexis, Westlaw.** `test/keizu/methods/test_sources.cljk` enforces this deny-list.
...
5. **Non-adjudicating (G2).** A source is read for facts and ties; keizu never imports a
   ...
   factual keizu kind or refused at the import boundary (`src/keizu/methods/bridge.cljk`).
